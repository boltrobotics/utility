...
{
    ASSERT_EQ(1, buff_.size());
    ASSERT_EQ(0, buff_.available());
    ASSERT_EQ(1, buff_.remaining());

    bool success = buff_.resize(5);
    buff_.write_ptr() += 3;

    ASSERT_EQ(true, success);
    ASSERT_EQ(5, buff_.size());
    ASSERT_EQ(3, buff_.available());
    ASSERT_EQ(2, buff_.remaining());
    ...
    ASSERT_EQ(4097, buff_.size());
    ASSERT_EQ(3, buff_.available());
    ASSERT_EQ(4097 - 3, buff_.remaining());
    ...
    ASSERT_TRUE(buff_.data() != NULL);
    ASSERT_TRUE(buff_.data() == data);
    ASSERT_EQ(4, buff_.size());
    ASSERT_EQ(3, buff_.available());
    ASSERT_EQ(4 - 3, buff_.remaining());
}
...
TEST_F(BuffTest, extend)
{
    ...
    ASSERT_EQ(3, buff_.size());
    ASSERT_EQ(0, buff_.available());
    ASSERT_EQ(3, buff_.remaining());

    bool success = buff_.extend(10000);
    buff_.write_ptr() += 3;

    ASSERT_EQ(true, success);
    ASSERT_EQ(10000, buff_.size());
    ASSERT_EQ(3, buff_.available());
    ASSERT_EQ(10000 - 3, buff_.remaining());

    buff_.reset();
    ASSERT_EQ(10000, buff_.size());
    ASSERT_EQ(0, buff_.available());
    ASSERT_EQ(10000, buff_.remaining());
}
...
TEST_F(BuffTest, readWriteChunk)
{
    ...
    ASSERT_EQ(6, buff_.size());
    ASSERT_EQ(6, buff_.available());
    ASSERT_EQ(0, buff_.remaining());
...
TEST_F(BuffTest, shiftOnWrite)
{
    ...
    ASSERT_EQ(true, success);
    ASSERT_EQ(0, buff_.consumed());
    ASSERT_EQ(2, buff_.available());
    ASSERT_EQ(1, buff_.remaining());
    ASSERT_EQ(3, buff_.size());
