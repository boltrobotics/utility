...
public:
  ...
  static IntType getNBitVarInt(Buff* buff, const IdxType (&bit_map)[N], bool move_ptr = true);
...
template<typename IntType, typename IdxType, uint32_t N>
inline IntType ValueCodec::getNBitVarInt(Buff* buff, const IdxType (&bit_map)[N], bool move_ptr)
{
  IntType val = 0;
...
template<typename IntType>
inline int ValueCodec::getFixedInt(Buff* buff, IntType* val, uint32_t bytes, bool msb)
{
